...
class EventRelationshipAnalyzer:
    ...
    def process_relationships(
        self, disclosures: List[Disclosure], analysis_results: Dict[str, AnalysisResult]
    ) -> None:
        """
        ...
        for current in sorted_discs:
            ...
            if current.semantic_core == "Önceki açıklamanın düzeltilmesi" or current.is_correction:
                ...
                if target:
                    current_res.related_disclosures.append(
                        f"Düzeltme hedefi: {target.disclosure_index}"
                    )
            ...
            if current.semantic_core == "Yeni iş bağlantısı/sözleşme":
                previous = self._find_previous_similar(
                    current, sorted_discs, "Yeni iş bağlantısı/sözleşme"
                )
                if previous:
                    current_res.related_disclosures.append(
                        f"Devam haberi niteliğinde: {previous.disclosure_index}"
                    )
                    if previous.disclosure_index in analysis_results:
                        analysis_results[previous.disclosure_index].related_disclosures.append(
                            f"Süreç sonucu: {current.disclosure_index}"
                        )
    ...
    def _find_correction_target(
        self, current: Disclosure, discs: List[Disclosure]
    ) -> Disclosure | None:
        # Heuristic: find immediately preceding disclosure of same title/type or by title match
        # Simplified for prototype
        for d in reversed(discs):
            if (
                d.publish_date < current.publish_date
                and d.disclosure_index != current.disclosure_index
            ):
                return d
        return None

    def _find_previous_similar(
        self, current: Disclosure, discs: List[Disclosure], core_type: str
    ) -> Disclosure | None:
        for d in reversed(discs):
