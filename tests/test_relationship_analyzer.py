...
def test_relationship_correction():
    now = datetime.now()
    d1 = Disclosure(disclosure_index="1", publish_date=now - timedelta(days=2), title="Hata", semantic_core="Rutin açıklama", importance=DisclosureImportance.MATERIAL)
    d2 = Disclosure(disclosure_index="2", publish_date=now - timedelta(days=1), title="Düzeltme", semantic_core="Önceki açıklamanın düzeltilmesi", is_correction=True)

    discs = [d1, d2]
    results = {
        "1": AnalysisResult(disclosure_id="1"),
        "2": AnalysisResult(disclosure_id="2")
    }
...
def test_relationship_continuation():
    now = datetime.now()
    d1 = Disclosure(disclosure_index="1", publish_date=now - timedelta(days=2), title="Görüşme", semantic_core="Yeni iş bağlantısı/sözleşme")
    d2 = Disclosure(disclosure_index="2", publish_date=now - timedelta(days=1), title="İmza", semantic_core="Yeni iş bağlantısı/sözleşme")
...
def test_relationship_contradiction_risk():
    now = datetime.now()
    d1 = Disclosure(disclosure_index="1", publish_date=now - timedelta(days=2), title="Yatırım", semantic_core="Kapasite artışı / yeni yatırım")
    d2 = Disclosure(disclosure_index="2", publish_date=now - timedelta(days=1), title="Kredi", semantic_core="Finansman / borçlanma")
