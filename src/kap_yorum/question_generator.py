...
class QuestionGenerator:
    ...
    def generate_and_resolve(self, disclosure: Disclosure) -> List[EconomicQuestion]:
        ...
        if "sözleşme" in core:
            questions.append(
                EconomicQuestion(
                    question="Kesinleşmiş mi?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Tutarı ne?", status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Şirket için önemli büyüklükte mi?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Kârlılık hakkında yeterli bilgi var mı?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Ek kapasite gerektiriyor mu?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            ...
            questions.append(
                EconomicQuestion(
                    question="Yatırımın toplam tutarı ne kadar?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Finansmanı nasıl sağlanacak (özkaynak/borç)?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Ne zaman devreye alınacak?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Beklenen ciro/kâr katkısı nedir?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            ...
            questions.append(
                EconomicQuestion(
                    question="Ciro/kâr büyümesi enflasyonun üzerinde mi?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            questions.append(
                EconomicQuestion(
                    question="Borçluluk oranı ne durumda?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
            ...
            questions.append(
                EconomicQuestion(
                    question="Şirket faaliyetlerine doğrudan ekonomik etkisi var mı?",
                    status=QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION,
                )
            )
        ...
        facts_text = (
            " ".join(disclosure.verified_facts).lower() if disclosure.verified_facts else ""
        )
        ...
        for q in questions:
            if q.question == "Kesinleşmiş mi?":
                ...
                if q.status == QuestionStatus.INSUFFICIENT_PUBLIC_INFORMATION and not q.reason:
                    q.reason = (
                        "Kamuya açıklanan metinde bu soruyu yanıtlayacak detay yer almamaktadır."
                    )
